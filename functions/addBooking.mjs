...
import { calculateBookingEndTime, isBookingAvailable } from "./bookingManagement.mjs";
...
export async function addBooking(req, res) {
  ...
  try {
    ...
    const {endTime: bookingEndTime, estimatedRideDurationHours} = calculateBookingEndTime(
      fromPincode,
      toPincode,
      startTime
    );

    if ((await isBookingAvailable(startTime, bookingEndTime)) === false) {
      return res
        .status(409)
